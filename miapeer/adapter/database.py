...
engine: Engine = create_engine(
    db_uri(), connect_args={"check_same_thread": False}, echo=False
)
...
def seed_db() -> None:  # pragma: no cover
    ...
    if (
        not superuser_username
        or not superuser_password
        ...
        with Session(engine) as db:
            ...
            db.add_all(
                [
                    miapeer_super_user := ApplicationRole(
                        application_id=miapeer.application_id
                        if miapeer.application_id
                        else 0,
                        role_id=super_user_role.role_id
                        if super_user_role.role_id
                        else 0,
                    ...
                    quantum_super_user := ApplicationRole(
                        application_id=quantum.application_id
                        if quantum.application_id
                        else 0,
                        role_id=super_user_role.role_id
                        if super_user_role.role_id
                        else 0,
            ...
            db.add_all(
                [
                    ...
                    admin := User(
                        email=admin_username, password=admin_password, disabled=False
                    ),
                ]
            )
            ...
            db.add_all(
                [
                    Permission(
                        ...
                        application_role_id=miapeer_super_user.application_role_id
                        if miapeer_super_user.application_role_id
                        else 0,
                    ),
                    Permission(
                        user_id=super_user.user_id if super_user.user_id else 0,
                        application_role_id=miapeer_admin.application_role_id
                        if miapeer_admin.application_role_id
                        else 0,
                    ),
                    Permission(
                        user_id=admin.user_id if admin.user_id else 0,
                        application_role_id=miapeer_user.application_role_id
                        if miapeer_user.application_role_id
                        else 0,
                    ),
                    Permission(
                        user_id=admin.user_id if admin.user_id else 0,
                        application_role_id=quantum_user.application_role_id
                        if quantum_user.application_role_id
                        else 0,
                    ),
                ]
            )
            ...
            db.add_all(
                [
                    RepeatOption(
                        name="Weekly",
                        repeat_unit_id=day.repeat_unit_id if day.repeat_unit_id else 0,
                        quantity=7,
                        order_index=1,
                    ),
                    RepeatOption(
                    ...
                    RepeatOption(
                        name="Monthly",
                        repeat_unit_id=month.repeat_unit_id
                        if month.repeat_unit_id
                        else 0,
                        quantity=1,
                        order_index=4,
                    ),
                    ...
                    RepeatOption(
                        name="Anually",
                        repeat_unit_id=year.repeat_unit_id
                        if year.repeat_unit_id
                        else 0,
                        quantity=1,
                        order_index=7,
                    ),
