...
@pytest.fixture
def basic_category(
    category_name: str, parent_category_id: int, portfolio_id: int
) -> Category:
    return Category(
        category_id=None,
        name=category_name,
...
def complete_category(category_id: int, basic_category: Category) -> Category:
    return Category.model_validate(
        basic_category.model_dump(), update={"category_id": category_id}
    )
...
class TestGetAll:
    ...
    @pytest.fixture
    def expected_multiple_categories(
        self, complete_category: Category
    ) -> list[CategoryRead]:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([], []),
            (
                lazy_fixture("multiple_categories"),
                lazy_fixture("expected_multiple_categories"),
            ),
        ],
    )
    async def test_get_all(
        ...
        mock_db: Mock,
        expected_sql: str,
        expected_response: list[CategoryRead],
    ) -> None:
...
class TestCreate:
    ...
    @pytest.fixture
    def category_to_create(
        self, category_name: str, parent_category_id: int, portfolio_id: int
    ) -> CategoryCreate:
    ...
    async def test_create_with_portfolio_found(
        ...
        await category.create_category(
            category=category_to_create, db=mock_db, current_user=user
        )
...
class TestGet:
    ...
    @pytest.mark.parametrize(
        "db_one_or_none_return_val", [lazy_fixture("complete_category")]
    )
    async def test_get_with_data(
        ...
        response = await category.get_category(
            category_id=category_id, db=mock_db, current_user=user
        )
...
class TestDelete:
    ...
    async def test_delete_with_category_found(
        ...
        response = await category.delete_category(
            category_id=category_id, db=mock_db, current_user=user
        )
    ...
    async def test_delete_with_category_not_found(
        ...
        with pytest.raises(HTTPException):
            await category.delete_category(
                category_id=category_id, db=mock_db, current_user=user
            )
...
class TestUpdate:
    ...
    def category_updates(self, parent_category_id: int) -> CategoryUpdate:
        return CategoryUpdate(
            name="some new name", parent_category_id=parent_category_id
        )
    ...
    def updated_category(self, complete_category: Category) -> Category:
        return Category.model_validate(
            complete_category.model_dump(), update={"name": "some new name"}
        )
