...
@pytest.fixture
def basic_application_role(
    application_id: int, role_id: int, application_role_description: str
) -> ApplicationRole:
...
@pytest.fixture
def complete_application_role(
    application_role_id: int, basic_application_role: ApplicationRole
) -> ApplicationRole:
    return ApplicationRole.model_validate(
        basic_application_role.model_dump(),
        update={"application_role_id": application_role_id},
    )


class TestGetAll:
    @pytest.fixture
    def multiple_application_roles(
    ...
    def expected_multiple_application_roles(
        ...
        working_application_role = ApplicationRoleRead.model_validate(
            complete_application_role
        )
        return [working_application_role, working_application_role]

    @pytest.fixture
    def expected_sql(self) -> str:
        return "SELECT miapeer_application_role.application_id, miapeer_application_role.role_id, miapeer_application_role.description, miapeer_application_role.application_role_id \nFROM miapeer_application_role"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([], []),
            (
                lazy_fixture("multiple_application_roles"),
                lazy_fixture("expected_multiple_application_roles"),
            ),
        ],
    )
    async def test_get_all(
        ...
        mock_db: Mock,
        expected_sql: str,
        expected_response: list[ApplicationRoleRead],
    ) -> None:
...
class TestCreate:
    ...
    @pytest.fixture
    def application_role_to_create(
        self, basic_application_role: ApplicationRole
    ) -> ApplicationRoleCreate:
    ...
    async def test_create(
        ...
        await application_role.create_application_role(
            application_role=application_role_to_create, db=mock_db
        )
...
class TestGet:
    ...
    @pytest.mark.parametrize(
        "db_get_return_val", [lazy_fixture("complete_application_role")]
    )
    async def test_get_with_data(
        ...
        response = await application_role.get_application_role(
            application_role_id=application_role_id, db=mock_db
        )
    ...
    async def test_get_with_no_data(
        self, application_role_id: int, mock_db: Mock
    ) -> None:
        with pytest.raises(HTTPException):
            await application_role.get_application_role(
                application_role_id=application_role_id, db=mock_db
            )
...
class TestDelete:
    ...
    async def test_delete_with_application_role_found(
        ...
        response = await application_role.delete_application_role(
            application_role_id=application_role_id, db=mock_db
        )
    ...
    @pytest.mark.parametrize("db_get_return_val", [None, []])
    async def test_delete_with_application_role_not_found(
        self, application_role_id: int, mock_db: Mock
    ) -> None:
        with pytest.raises(HTTPException):
            await application_role.delete_application_role(
                application_role_id=application_role_id, db=mock_db
            )
...
class TestUpdate:
    ...
    @pytest.fixture
    def updated_application_role(
        self, complete_application_role: ApplicationRole
    ) -> ApplicationRole:
        return ApplicationRole.model_validate(
            complete_application_role.model_dump(),
            update={"description": "some new description"},
        )

    @pytest.fixture
    def expected_response(
        self, updated_application_role: ApplicationRole
    ) -> ApplicationRoleRead:
