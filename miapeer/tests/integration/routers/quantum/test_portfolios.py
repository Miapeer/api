...
class TestGetAll:
    def test_get_all_user_portfolios(
        self, client: TestClient, my_portfolio: Portfolio
    ) -> None:
        response = client.get(
...
class TestCreate:
    def test_create_portfolio(
        self, client: TestClient, not_my_portfolio: Portfolio
    ) -> None:
        ...
        assert response.json() == {
            ...
            + 1,  # Increment by 1, the last portfolio ID inserted
        }


@pytest.mark.usefixtures("create_complete_portfolio")
class TestGetOne:
    def test_get_one_portfolio_succeeds(
        self, client: TestClient, my_portfolio: Portfolio
    ) -> None:
    ...
    def test_get_one_portfolio_that_is_not_mine_fails(
        self, client: TestClient, not_my_portfolio: Portfolio
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "portfolio_id", [0, -1, 999999999999999999, -999999999999999999]
    )
    def test_get_one_account_with_invalid_portfolio_id_fails(
        self, client: TestClient, portfolio_id: int
    ) -> None:
