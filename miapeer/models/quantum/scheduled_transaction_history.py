...
class ScheduledTransactionHistoryBase(SQLModel):
    target_date: date
    post_date: date
    scheduled_transaction_id: int = Field(
        foreign_key="quantum_scheduled_transaction.scheduled_transaction_id"
    )
...
class ScheduledTransactionHistory(ScheduledTransactionHistoryBase, table=True):
    ...
    scheduled_transaction_history_id: Optional[int] = Field(
